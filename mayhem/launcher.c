 ...
 * -DHARNESS=... and -DPYTHON=... so the binary is self-contained. Running
 * `python3 <harness>` puts the harness's own directory on sys.path[0], so the
 * harness's `import fuzz_helpers` (a sibling module) resolves.
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
#endif
#ifndef HARNESS
#define HARNESS "/mayhem/mayhem/fuzz_load.py"
#endif
